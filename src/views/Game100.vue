...
socket.on(`selected_card_respose_${stake}`, (games) => {
  ...
  const a = game.value.players.find((p) => p.user_id === phone.value);
});
...
<template>
  <div v-if="!game?.active">
    <Navbar :wallet="realBalance + bonusBalance" :game="game" :stake="100" />
  ...
  <div v-if="game?.active">
    ...
    <div class="flex h-[100%] justify-center mt-2 gap-1 py-2">
      ...
      <div class="flex-1">
        <div
          class="bg-white rounded-lg border-2 border-gray-300 flex-1 h-14 flex justify-start items-center text-lg font-bold text-gray-100 bg-opacity-50 px-2 uppercase tracking-wider"
        >
