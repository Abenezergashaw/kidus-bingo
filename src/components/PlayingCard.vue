<script setup>
import { computed, ref } from "vue";
const props = defineProps({
  card: {
    type: Object,
    required: true,
  },
  game: Object,
  phone: String,
  errorMessage: String,
});

const userSelectedNumbers = ref([]);

function handleNumberSelect(n) {
  const index = userSelectedNumbers.value.indexOf(n);

  if (index !== -1) {
    userSelectedNumbers.value.splice(index, 1);
    return;
  }

  userSelectedNumbers.value.push(n);
}

const status = computed(() => {
  const player = props.game.players.find((p) => p.user_id === props.phone);
  return player ? player.blocked : false;
});
</script>

<template>
  <div class="mt-4 p-2 border rounded-xl shadow-md">
    <div class="text-lg text-gray-200 text-center tracking-wider my-1">
      Cartela {{ card.id }}
    </div>
    <div class="grid grid-cols-5 gap-1 text-center font-bold text-xl mb-2">
      <div
        v-for="letter in ['B', 'I', 'N', 'G', 'O']"
        :key="letter"
        class="rounded-lg text-white font-bold backdrop-blur-lg"
        :class="`${
          letter === 'B'
            ? 'bg-[#E33739]'
            : letter === 'I'
            ? 'bg-[#FDB300]'
            : letter === 'N'
            ? 'bg-[#4BA04D]'
            : letter === 'G'
            ? 'bg-[#1D8AEB]'
            : letter === 'O'
            ? 'bg-[#7E3098]'
            : 'bg-white'
        }`"
      >
        {{ letter }}
      </div>
    </div>
    <div class="grid grid-cols-5 gap-1">
      <div
        v-for="key in ['b1', 'i1', 'n1', 'g1', 'o1']"
        :key="key"
        class="flex items-center justify-center w-8 h-8 text-lg font-semibold text-gray-800 border-2 border-white rounded-lg backdrop-blur-md transition-all duration-300"
        @click="handleNumberSelect(card[key])"
        :class="`${
          game?.current_number === card[key]
            ? 'animate-pulse'
            : userSelectedNumbers.includes(card[key])
            ? 'bg-teal-500 text-white'
            : 'bg-gray-200'
        }`"
      >
        {{ card[key] }}
      </div>

      <div
        v-for="key in ['b2', 'i2', 'n2', 'g2', 'o2']"
        :key="key"
        @click="handleNumberSelect(card[key])"
        class="flex items-center justify-center w-8 h-8 text-lg font-semibold text-gray-800 border-2 border-white rounded-lg backdrop-blur-md transition-all duration-300"
        :class="`${
          game?.current_number === card[key]
            ? 'animate-pulse'
            : userSelectedNumbers.includes(card[key])
            ? 'bg-teal-500 text-white'
            : 'bg-gray-200'
        }`"
      >
        {{ card[key] }}
      </div>

      <div
        v-for="key in ['b3', 'i3', 'n3', 'g3', 'o3']"
        :key="key"
        @click="handleNumberSelect(card[key])"
        class="flex items-center justify-center w-8 h-8 text-lg font-semibold text-gray-800 border-2 border-white rounded-lg backdrop-blur-md transition-all duration-300"
        :class="`${
          game?.current_number === card[key]
            ? 'animate-pulse'
            : userSelectedNumbers.includes(card[key])
            ? 'bg-teal-500 text-white'
            : 'bg-gray-200'
        }`"
      >
        <span v-if="key === 'n3'" class="text-gray-500 font-medium">★ </span>
        <span class="text-lg font-semibold" v-else>{{ card[key] }}</span>
      </div>

      <div
        v-for="key in ['b4', 'i4', 'n4', 'g4', 'o4']"
        :key="key"
        @click="handleNumberSelect(card[key])"
        class="flex items-center justify-center w-8 h-8 text-lg font-semibold text-gray-800 border-2 border-white rounded-lg backdrop-blur-md transition-all duration-300"
        :class="`${
          game?.current_number === card[key]
            ? 'animate-pulse'
            : userSelectedNumbers.includes(card[key])
            ? 'bg-teal-500 text-white'
            : 'bg-gray-200'
        }`"
      >
        {{ card[key] }}
      </div>

      <div
        v-for="key in ['b5', 'i5', 'n5', 'g5', 'o5']"
        :key="key"
        @click="handleNumberSelect(card[key])"
        class="flex items-center justify-center w-8 h-8 text-lg font-semibold text-gray-800 border-2 border-white rounded-lg backdrop-blur-md transition-all duration-300"
        :class="`${
          userSelectedNumbers.includes(card[key])
            ? 'bg-teal-500 text-white'
            : game?.current_number === card[key]
            ? ' bg-teal-500 text-white animate-pulse'
            : 'bg-gray-200'
        }`"
      >
        {{ card[key] }}
      </div>
    </div>
  </div>
  <div
    v-if="errorMessage"
    class="text-center text-red-500 tracking-wide text-lg mt-1"
  >
    {{ errorMessage }}
  </div>
  <button
    @click="$emit('handleBingo', card.id)"
    class="w-full py-4 rounded-xl my-2 text-xl font-bold tracking-wider border border-white flex justify-center items-center"
    :class="`${status ? 'bg-red-500 text-white' : 'bg-amber-300 text-black'}`"
  >
    BINGO!
  </button>
</template>

<style scoped>
.bingo-cell {
  @apply flex items-center justify-center w-8 h-8 text-lg font-semibold text-gray-800 bg-gray-200 border-2 border-white rounded-lg backdrop-blur-md transition-all duration-300;
}
</style>
